...
1. ** 文本大模型推理 **： 2023-至今，负责内部自研大模型推理框架的总体设计；MoE推理优化；投机推理的算法和工程；算法-工程协同设计模型结构。
2. ** 多模态大模型推理 **：2023-至今，负责语音（TTS+ASR）、融合多模（图文理解、图文生成、语音融合多模）等多模态大模型的推理架构。
3. ** 多模态大模型RL **：2025-至今，负责多模态RL的rollout优化过程。
4. ** 多模态大模型训练 **：2023-2024，融合多模训练。
5. ** 模型量化压缩 **：2025-至今，模型压缩方向，模型稀疏化，高效Attention结构设计。
6. ** 国产芯片推理 **：2025-至今，负责昇腾卡的推理落地，在极致时延上，对标H20。
7. ** 端侧推理引擎 **：2025-至今，负责公司级的端侧推理引擎和IoT场景的支持。
...
## 论文
1. **Team, Meituan LongCat et al.** “LongCat-Flash-Thinking Technical Report.” (2025).
2. **Team, Meituan LongCat et al.** “LongCat-Flash Technical Report.” ArXiv abs/2509.01322 (2025): n. pag.
3. **Li, Q., Zhang, B., Kang, H., Xu, T., Qian, Y., Xie, Y., & Ma, L. (2025).** FlashCommunication V2: Bit Splitting and Spike Reserving for Any Bit Communication. ArXiv, abs/2508.03760.
4. **Su, Z., Li, Q., Zhang, H., Qian, Y., Xie, Y., & Yuan, K. (2025).** Unveiling Super Experts in Mixture-of-Experts Large Language Models. ArXiv, abs/2507.23279.
5. **Qian, Y., Li, F., Ji, X., Zhao, X., Tan, J., Zhang, K., & Cai, X. (2024).** EPS-MoE: Expert Pipeline Scheduler for Cost-Efficient MoE Inference. ArXiv, abs/2410.12247.
6. **Zhao, W., Zhang, J., Xie, D., Qian, Y., Jia, R., & Li, P. (2019, November).** AIBox: CTR prediction model training on a single node. In Proceedings of the 28th ACM International Conference on Information and Knowledge Management (pp. 319-328).
7. **Zhao, Weijie, Deping Xie, Ronglai Jia, Yulei Qian, Ruiquan Ding, Mingming Sun, and Ping Li. (2020).** Distributed hierarchical gpu parameter server for massive scale deep learning ads systems. Proceedings of Machine Learning and Systems 2 (2020): 412-428.
